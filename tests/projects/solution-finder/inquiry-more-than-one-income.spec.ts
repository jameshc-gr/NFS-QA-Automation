...
const inquiryConfig = {
  ...
  applicationInquiryInput: {
    ...
    incomeInput: [
      ...
      {
        annualIncome: 100000,
        incomeSource: "EMPLOYED_PART_TIME",
        incomeType: null,
      },
    ],
    loId: 6068,
...
test("@smoke NewInquiryPage - multiple income sources - all offers eligible", async ({
  ...
  for (let addlIndex = 0; addlIndex < additionalIncomes.length; addlIndex++) {
    ...
    // Click "Add additional income source"
    ...
    // The new additional income row contains input[name="income-<index>"]
    ...
    // Additional income source dropdown lives in the same row
    ...
    if (
      ...
      // Fallback: custom dropdown component
  ...
  // HELOC offers (cards 0 and 1) are eligible: each shows a rate, loan amount, term and details
