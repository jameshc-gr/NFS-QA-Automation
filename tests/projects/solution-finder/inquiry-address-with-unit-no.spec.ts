...
test("@smoke NewInquiryPage - Address with unit number(APT E)", async ({
  page,
}) => {
  ...
  const heloanRate = card.getByText(/^\d+\.\d{2}%$/).first();
  if (await heloanRate.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    await expect(card).toContainText(
      "HELOAN application creation failed for inquiry",
    );
  }
