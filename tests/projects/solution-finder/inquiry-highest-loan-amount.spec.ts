...
test(
  "@smoke NewInquiryPage - highest requested loan amount - HELOC capped HELOAN ineligible",
  async ({ page }) => {
    ...
    // High requested loan amount ($800,000) — the key driver of ineligibility
    ...
    // ──────────────────────────────────────────────────────────────────
    // Validate Offers UI ($800K request — HELOC capped to max eligible, HELOAN ineligible)
    // ──────────────────────────────────────────────────────────────────
    ...
    // Fixed Rate HELOC (card 0) — QA returns eligible offer (capped to max loan amount)
    const fixedHelocCard = page.getByTestId("inquiry-offer-card-0");
    await expect(fixedHelocCard.getByText(/^\d+\.\d{2}%$/).first()).toBeVisible(
      { timeout: 5000 },
    );
    ...
    // HELOAN (card 2) — ineligible due to loan amount / LTV thresholds (exact reason varies in QA)
