...
const inquiryConfig = {
  ...
  applicationInquiryInput: {
    ...
    basicInfoInput: {
      emailId: "test-cond-na@yopmail.com",
      name: {
        first: "Erica",
        middle: "",
        last: "Lambert",
...
test(
  "@smoke NewInquiryPage - condominium property type - HELOC ineligible HELOAN eligible",
  async ({ page }) => {
    ...
    // ──────────────────────────────────────────────────────────────────
    // Validate Offers UI (condominium — HELOC ineligible, HELOAN eligible)
    // ──────────────────────────────────────────────────────────────────
    ...
    // HELOC offers (cards 0 and 1) are ineligible — no property found for condominium
    for (const cardIndex of [0, 1]) {
      ...
      await expect(
        page.getByTestId(`inquiry-offer-card-${cardIndex}`),
      ).toContainText(/NO PROPERTY FOUND|WAITING_FOR_LIEN_DATA|NAV_RESI_EVAL_ESTIMATE_NO_AVM/);
    }
