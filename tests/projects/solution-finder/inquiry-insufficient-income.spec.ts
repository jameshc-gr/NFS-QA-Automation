...
const inquiryConfig = {
  ...
  applicationInquiryInput: {
    ...
    basicInfoInput: {
      emailId: "test-less-na-income@yopmail.com",
      name: {
        first: "Erica",
        middle: "",
        last: "Lambert",
...
test(
  "@smoke NewInquiryPage - insufficient income - HELOC ineligible HELOAN eligible",
  async ({ page }) => {
    ...
    // Annual Income (low: $1,000 to trigger insufficient income scenario)
    ...
    // ──────────────────────────────────────────────────────────────────
    // Validate Offers UI (insufficient income — HELOC ineligible, HELOAN eligible)
    // ──────────────────────────────────────────────────────────────────
    ...
    await expect(heloanCard).toContainText("HELOAN");
     ...
     // Validate HELOAN shows DTI-related rejection reasons based on income and Requested loan amount
    await expect(heloanCard).toContainText(ineligibleHeading);
    await expect(heloanCard).toContainText("DTI Ratio > 50");
