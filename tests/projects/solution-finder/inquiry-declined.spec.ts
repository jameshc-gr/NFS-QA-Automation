...
const inquiryConfig = {
  ...
  applicationInquiryInput: {
    ...
    basicInfoInput: {
      emailId: "test-ximnna-2323@yopmail.com",
      name: {
        first: "Erica",
        middle: "",
        last: "Lambert",
...
test(
  "@smoke NewInquiryPage - declined - HELOC ineligible HELOAN eligible",
  async ({ page }) => {
    ...
    // ──────────────────────────────────────────────────────────────────
    // Validate Offers UI (declined — HELOC ineligible, HELOAN eligible)
    // ──────────────────────────────────────────────────────────────────
    ...
    // HELOC offers (cards 0 and 1) are ineligible — property value or collateral not sufficient
    for (const cardIndex of [0, 1]) {
      ...
      await expect(
        page.getByTestId(`inquiry-offer-card-${cardIndex}`),
      ).toContainText("Property value or type of collateral not sufficient");
    }
