...
test("@smoke NewInquiryPage - credit freeze - all offers ineligible", async ({
  ...
  // Credit file is frozen, so no FICO scores are returned (shown as "Unknown")
  const ficoValue = offersPage.locator('span:text-is("FICO:") + span').first();
  await expect(ficoValue).toContainText("Unknown");
  ...
  // With a frozen credit file, every product is ineligible.
