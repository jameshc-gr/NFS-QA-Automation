...
test("@smoke NewInquiryPage - low experian credit score", async ({
  ...
  for (const cardIndex of [0, 1]) {
    ...
    ).toContainText("Your credit score is below our minimum requirement");
  }

  // HELOAN (card 2) is ineligible because the FICO score is below 680
  await expect(page.getByTestId("inquiry-offer-card-2")).toContainText(
    "TU Soft pull was not successful",
  );
