...
describe('iOS create account with email + SMS verification', () => {
  it('creates a new account and completes email and SMS verification into the home page', async function () {
    ...
    try {
      ...
      // Step 6: iOS offers Face ID enrolment before onboarding, then closes with
      // the survey banner that hides the home dashboard.
      await home.skipBiometricsSetupIfPresent(30000);

      // The banner is not shown on every run, so close it when it is there and
      // let the home dashboard check decide whether the run succeeded.
